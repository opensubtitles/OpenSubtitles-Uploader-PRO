...
export const useFileHandling = (addDebugInfo, config = {}) => {
  ...
  useEffect(() => {
    ...
    const currentFilesSignature = files
      .map(
        f =>
          f.fullPath +
          f.size +
          (f.movieHash || 'null') +
          (f.detectedLanguage ? JSON.stringify(f.detectedLanguage) : 'null')
      )
      .join('|');
    ...
    async event => {
      ...
      try {
        ...
        if (isTauriEnv) {
          ...
          if (
            !event.dataTransfer ||
            (!event.dataTransfer.files?.length && !event.dataTransfer.items?.length)
          ) {
        ...
        console.log(
          '📁 Collected files:',
          collectedFiles.map(f => ({ name: f.name, path: f.fullPath, type: f.type }))
        );
        ...
        if (collectedFiles.length > 0) {
          ...
          const allMkvFiles = collectedFiles.filter(file =>
          ...
          allMkvFiles.forEach(file => {
            console.log(
              `🔍 MKV file: ${file.name} - hasMkvSubtitleExtraction: ${file.hasMkvSubtitleExtraction}, isVideo: ${file.isVideo}`
            );
          });

          if (mkvFiles.length > 0) {
            addDebugInfo(
              `Starting MKV subtitle detection and auto-extraction for ${mkvFiles.length} file(s)`
            );
