...
export const SubtitleUploadOptions = ({
  ...
  const toggleExpanded = compactMode
    ? onToggleExpanded
  ...
  const checkForeignPartsFromString = str => {
    ...
    const foreignPartsRegex =
      /\[?forced\]?|foreign[.\s_-]?parts?|\[?partial\]?|alien[.\s_-]?language|native[.\s_-]?only|parts[.\s_-]?only|non[.\s_-]*english[.\s_-]?parts?|language[.\s_-]?only|\[?commentary\]?/i;
  ...
  useEffect(() => {
    ...
    uploadOptions?.moviereleasename,
    uploadOptions?.subauthorcomment,
    uploadOptions?.subtranslator,
    uploadOptions?.movieaka,
    uploadOptions?.hearingimpaired,
    uploadOptions?.highdefinition,
    uploadOptions?.foreignpartsonly,
    uploadOptions?.automatictranslation,
  ]);
  ...
  useEffect(() => {
    ...
    pairedVideoFile?.fullPath,
    subtitleContent,
    hasSetHearingImpaired,
    hasSetHighDefinition,
    hasSetForeignParts,
    hasSetAutoTranslation,
  ]);
  ...
  useEffect(() => {
    ...
    if ((subtitleFile || pairedVideoFile) && !hasSetReleaseName) {
      ...
      const releaseNameWithoutExtension = detectionName.replace(
        /\.(mp4|mkv|avi|mov|wmv|flv|webm|m4v|3gp|srt|ass|ssa|vtt|sub|idx|sup)$/i,
        ''
      );
  ...
  const handleFieldChange = useCallback(
    ...
    [
      uploadOptions,
      subtitlePath,
      localReleaseNameValue,
      localCommentValue,
      localTranslatorValue,
      localMovieAkaValue,
    ]
  );
  ...
  const ExpandedContent = React.useMemo(
    () => (
      <div
        className="mt-2 p-3 rounded border space-y-2"
        style={{
        ...
        <div className="flex flex-col gap-1 relative" onClick={e => e.stopPropagation()}>
          <div className="flex items-center gap-2">
            ...
            {config.defaultTranslator &&
              currentOptions.subtranslator === config.defaultTranslator &&
              !translatorError && (
                <div
                  className="absolute top-0 right-0 -mt-1 -mr-1 px-1 py-0.5 text-xs rounded-full text-white flex items-center gap-1"
                  style={{
          ...
          {translatorError && (
            <div className="text-xs text-red-500 ml-[84px]">{translatorError}</div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2" onClick={e => e.stopPropagation()}>
          <label className="flex items-center gap-1 cursor-pointer">
            <input
              type="checkbox"
              checked={
                currentOptions.automatictranslation === '1' || localAutoTranslationValue === '1'
              }
              onChange={e =>
                handleFieldChange('automatictranslation', e.target.checked ? '1' : '0')
              }
              className="w-3 h-3"
              style={{ accentColor: colors.link }}
            />
