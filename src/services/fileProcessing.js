...
export class FileProcessingService {
  ...
  static async processFileHandle(item, output, path = '', config = {}) {
    try {
      ...
      if (file.name.toLowerCase().endsWith('.mkv')) {
        ...
        console.log(
          `🔍 File classification: isVideo=${isVideo}, isSubtitle=${isSubtitle}, isMedia=${isMedia}`
        );
      }
      ...
      if (isMedia) {
        ...
        if (isVideo && file.name.toLowerCase().endsWith('.mkv')) {
          ...
          const extractMkvSubtitles =
            config.extractMkvSubtitles === true || config.extractMkvSubtitles === 'true';

          console.log(`🔍 MKV Config Debug for ${file.name}:`);
          console.log(
            `   - config.extractMkvSubtitles: ${config.extractMkvSubtitles} (type: ${typeof config.extractMkvSubtitles})`
          );
          ...
          if (extractMkvSubtitles) {
            ...
            console.log(
              `🎬 Detected MKV file: ${file.name}, will extract embedded subtitles using v1.8.1 API...`
            );
            console.log(
              `📺 Auto-extraction enabled - embedded subtitles will be detected and paired automatically`
            );
          } else {
   ...
   */
  static async processMkvExtractions(
    files,
    onFileUpdate,
    onSubtitleExtracted,
    addDebugInfo = null,
    ...
    if (!config.extractMkvSubtitles) {
      console.log(
        `⚠️ MKV subtitle extraction is disabled in configuration - skipping MKV processing`
      );
      if (addDebugInfo) {
    ...
    console.log(
      `🎬 Processing ${mkvFiles.length} MKV file(s) for subtitle extraction (enabled in config)`
    );
    ...
    for (const mkvFile of mkvFiles) {
      try {
        ...
        console.log(
          `🎯 Starting MKV subtitle extraction for: ${mkvFile.name} using v1.8.1 native API`
        );
        console.log(
          `🚀 Using direct extractAllSubtitles() - no separate metadata detection to avoid double file read`
        );
        ...
        try {
          ...
          console.log(
            `🔄 Calling v1.8.1 native extractAllSubtitles() function (single file read)...`
          );
        ...
        console.log(
          `🎉 Found and extracted ${extractionResult.extractedFiles.length} subtitle files`
        );

        // Also add to debug panel
        if (addDebugInfo) {
          addDebugInfo(
            `🎉 Extracted ${extractionResult.extractedFiles.length} subtitle files from ${mkvFile.name}`
          );
        }
        ...
        console.log(
          `🎯 Processing ${extractionResult.extractedFiles.length} extracted subtitle files...`
        );
        if (addDebugInfo) {
        ...
        try {
          ...
          if (addDebugInfo) {
            addDebugInfo(
              `🎯 Processing ${extractionResult.extractedFiles.length} extracted files...`
            );
          }
          ...
          for (const extractedFileData of extractionResult.extractedFiles) {
            ...
            if (extractedFileData && extractedFileData.file) {
              ...
              extractedFileData &&
              (extractedFileData.name || extractedFileData.filename) &&
              extractedFileData.size !== undefined
            ) {
            ...
            if (subtitleFile) {
              ...
              let originalName =
                extractedFileData.filename ||
                extractedFileData.name ||
                subtitleFile.name ||
                `stream_${extractedCount}.srt`;
              ...
              console.log(
                `📝 Generated subtitle name: ${subtitleName} (stream ${streamIndex}, language ${langCode})`
              );
              ...
              if (addDebugInfo) {
                addDebugInfo(
                  `⚠️ Skipped invalid subtitle data: ${extractedFileData?.filename || extractedFileData?.name || 'unknown'} (unsupported structure)`
                );
              }
            }
          }
        ...
        console.log(
          `🎉 v1.8.1 native extraction completed: ${extractedCount}/${extractionResult.extractedFiles.length} subtitles extracted and paired`
        );
        if (addDebugInfo) {
          addDebugInfo(
            `🎉 v1.8.1 extraction completed: ${extractedCount}/${extractionResult.extractedFiles.length} subtitles extracted and paired`
          );
        }
  ...
  static getBrowserCapabilities() {
    ...
    const isBrave =
      ...
      (userAgent.includes('Chrome') && !userAgent.includes('Edg') && !window.chrome?.webstore);
